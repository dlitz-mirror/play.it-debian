...
# USAGE: error_application_exe_empty $application
error_application_exe_empty() {
	...
	case "${LANG%_*}" in
		('fr')
			message='%s nʼest pas défini, mais cette valeur est requise.\n'
		;;
		('en')
			message='%s is not set, but is required.\n'
		;;
	esac

	print_error
	printf "$message" "${application}_EXE"
}
